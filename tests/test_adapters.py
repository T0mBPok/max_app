...
from app.imports.adapters import AquatikaAdapter, CultureRuAdapter, TomskPfdoAdapter, utc_datetime
from app.imports.base import content_hash, fallback_external_id
from app.models import ActivityType


def test_culture_normalization():
    item = CultureRuAdapter().normalize({"identifier": "42", "name": "Спектакль", "url": "https://culture.ru/events/42", "startDate": "2026-10-01T19:00:00+07:00", "offers": {"price": 0}})
...
def test_pfdo_normalization():
    item = TomskPfdoAdapter().normalize({"id": "7", "url": "/program/7", "title": "Робототехника", "text": "Для детей 8–12 лет"})
...
def test_hash_and_fallback_id_are_stable():
    ...
    assert fallback_external_id("HTTPS://EXAMPLE.COM/a/?utm=x") == fallback_external_id("https://example.com/a")
