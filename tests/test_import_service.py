...
from app.imports.base import ActivitySourceAdapter, NormalizedActivity
from app.imports.service import run_import
from app.models import Activity, ActivityStatus, ActivityType, SourceRecord, now_utc


class FakeAdapter(ActivitySourceAdapter):
    source_code = "fake"; name = "Fixture"; base_url = "https://example.test"

    def __init__(self): self.items = [{"id": "1", "title": "Кружок"}]

    async def fetch(self): return self.items

    def normalize(self, item):
        if "title" not in item: raise ValueError("bad fixture")
        return NormalizedActivity(
            external_id=item["id"], source_url=f"{self.base_url}/{item['id']}",
            type=ActivityType.SECTION, title=item["title"], category_slug="образование",
            schedule_text="Пн 18:00", raw_payload=item,
        )
