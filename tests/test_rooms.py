...
from app.room_service import create_room, join_locked, room_out
...
async def test_room_owner_age_join_and_capacity(session):
    ...
    session.add_all([category, owner, peer, adult]); await session.flush()
    activity = Activity(type=ActivityType.SECTION, title="Секция", category_id=category.id,
        city="Томск", status=ActivityStatus.ACTIVE)
    session.add(activity); await session.commit()
    room = await create_room(session, owner, RoomCreate(activity_id=activity.id, title="Идём вместе",
        age_preset=AgePreset.YOUTH_18_24, capacity=2, join_policy=JoinPolicy.OPEN,
        meeting_at=now_utc() + timedelta(days=1), meeting_point="У входа"))
