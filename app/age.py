...
from app.models import AgePreset
...
def validate_age_range(min_age: int, max_age: int | None) -> None:
    if min_age < 0 or (max_age is not None and max_age < min_age) or (min_age < 18 <= (max_age or min_age)):
...
def resolve_age_range(preset: AgePreset, min_age: int | None, max_age: int | None) -> tuple[int, int | None]:
